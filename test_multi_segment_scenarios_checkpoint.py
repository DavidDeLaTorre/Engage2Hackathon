...
def main():
    ...
    month = 11
    day = 16
    output_name = 'output/test_scenarios_checkpoint'

    # Define a cache file path (adjust folder as needed)
    cache_file = f"output/test_scenarios_checkpoint_cached_df_{year}_{month:02d}_{day:02d}.pkl"
    ...
    if os.path.exists(cache_file):
        ...
        df_list = []
        folder_paths = glob.glob("engage-hackaton-checkpoint/scenarios/*.parquet")
        for file in folder_paths:
            # Match all parquet files in the folder (e.g., "*.snappy.parquet" if needed)
            df = load_and_process_parquet_files([file], columns_to_extract=['df', 'icao24', 'ts', 'altitude', 'lat_deg',
                                                                           'lon_deg'])
            df_list.append(df)
    ...
    # Define a cache file path (adjust folder as needed)
    cache_file2 = f"output/test_scenarios_checkpoint_df_{year}_{month:02d}_{day:02d}.pkl"
    ...
    cache_file3 = f"output/test_scenarios_checkpoint_cached_landing_{year}_{month:02d}_{day:02d}.pkl"
    ...
    print("export_trajectories_to_kml_scenarios_icaos all ...")
    for icao in ["484fde", "344197", "3434ce", "4d2491", "347588", "71002a", "3453c9", "34604d", "0100dd", "3442cc", "020110", "3475c9", "344449", "34745a", "3444c3", "39e689", "7812a1", "344218", "346147", "342211"]:
        export_trajectories_to_kml_by_icao24(df, icao, output_name + icao +'_all.kml')
